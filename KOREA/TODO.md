...
## 🆕 2026-08-03 — 알림 3단계 + 일일 요약 / 데이터 전면 점검

### 텔레그램 알림 정책 변경
- **배경**: 매일 오는 성공 알림에 무뎌져 정작 중요한 걸 놓친다(사용자 피드백)
- **"실패만 보내기"는 채택 안 함** — 침묵이 *정상*인지 *죽어서 못 보냄*인지 구분되지 않고, 7/29 외인 부분수집(1,415/2,645)은 **성공 알림 안에 섞여 있던 문제**라 실패만 봤으면 영영 못 봤다
- [x] **3단계**: 실패=즉시+소리 / 이상 있는 성공=즉시(무음, 제목에 `·확인필요`) / 깨끗한 성공=**전송 안 함**
  - `warn=True` 파라미터 + detail의 `⚠️`/`⛔` **자동 감지** → 기존 호출부 일괄 수정 불필요
- [x] **일일 요약 (매일 11:30, 최근 24h)** — `logs/job_events.jsonl`(30일 보관)에 전 잡 결과 적재 후 한 통으로 발송
  - **이상 없어도 매일 보낸다** — 요약이 안 오는 것 자체가 이상 신호가 되게. 기록 0이면 `⚠️ 기록된 잡 없음 — 스케줄러 미가동 의심`
  - 11:30인 이유: 08:30 아침 종합 보충이 늦으면 10:50까지 감(실측). 24h 창이라 전날 23:30 stockfut부터 포함
  - "확인 필요" 항목은 detail **첫 줄이 아니라 `⚠️`/`⛔`/`에러` 있는 줄**을 고른다 (첫 줄은 보통 정상 요약이라 경고가 묻힘 — 만들다 발견해 수정)
- 효과: 하루 4~5통 → **1통** + 문제 있을 때만 추가. 상세는 `docs/스케줄러_운영.md`

### 데이터 전면 점검 결과 (2026-08-03)
정상 — 핵심 테이블 전부 **7/31**(직전 거래일) 기준. 8/1~8/2 주말이라 맞음.

| 테이블 | 최신 | 30초봉 일별 |
|---|---|---|
| ohlcv_daily / investor_trading / foreign_ownership / market_cap_daily | 2026-07-31 | 1.566~1.572M행/일 |
| index_ohlcv_daily / futures_ohlcv_daily / etf_portfolio_daily | 2026-07-31 | 선물 550~552계약 |
...
### 실행 결과 (전부 완료, 2026-08-02)
- [x] 복구(SQL, LS 호출 0) — `_adj_repair_run.py 20240423 20260729`: **368일자 / 25,931행 / 19분**. B·C·D 전부 0
- [x] 과거 백필(LS) — `--from 20220103 --to 20240422`: **1,672,971행 / 276분 / 에러 0**
- [x] 최근 구간 재정렬(LS) — `--from 20240423 --to 20260731`: **1,959,172행 / 477분 / 에러 0** (아래 stale factor 건)
- [x] 상폐 46종목 — **44종목 빈 응답**(LS가 상폐 종목 미제공). 2종목만 적재

### ⚠️ 조사 중 추가 발견 — 2024-04-23 이후 factor가 stale이었다
- 경계 점검에서 2024-04-22/23 factor 불연속 **418종목** 발견. 처음엔 "내 백필이 edate 기준으로 정규화를 어긋나게 썼다"고 의심했으나 **가설이 틀림** — edate를 바꿔도 LS adj 값은 동일(실측). 방향이 반대였다
- 실제 원인: **2026-05-16 백필 이후 발생한 액면병합·감자가 2024-04-23 이후 구간에 미반영**. 신규 백필이 신선한 값을 옆에 놓으면서 드러남
  ```
  LS 현재: 042040  04-22~04-25 전부 factor 80.1 (연속)
  DB 기존: 04-22 80.1(신규) / 04-23~ 10.0(2026-05-16) ← stale
  ```
- **`latest factor = 1.0` 불변식으로는 탐지 불가** — 3,929종목 전부 만족하는데도 스케일이 어긋나 있었다. DB 시리즈가 내부적으로 일관되기 때문. 이벤트를 통째로 하나 놓치면 조용히 어긋난다
- [x] 전 종목 2024-04-23~2026-07-31 재정렬 → 불연속 **상대 0.1% 초과 0건** (최대 상대차 0.08%). 잔여 절대차 10건은 `adj_close` 정수 반올림 양자화 오차 (factor 큰 종목에서 절대 기준이 부적절 — **판정은 상대 기준으로**)
- **LENS 캐시 재적재 필요** — 2024-04-23 이후 adj_close 값이 바뀌었음

### 최종 상태
| 지표 | 이전 | 현재 |
|---|---|---|
| 3년창 NULL | 604,971 (23.7%) | **7,033 (0.28%)** |
| 대표 6종목 | 726봉 중 178 NULL | **726봉 / NULL 0** |
| 6/2~6/9 | 일 200건 NULL | **0** |
| 만성 ETF 0082V0/0025N0 | 139/59일 | **0/0** |

- 잔여 = **상폐 44종목**의 2022-01-03~2024-04-22 (종목당 34~567행, 평균 380 / 3년창 7,033행, 전 기간 16,700행). LS 미제공이라 복구 불가 — 생존편향 백테스트 시 원주가만 신뢰 가능
- 상폐 44종목의 2024-04-23 이후 구간은 재정렬 대상(활성)에서 빠져 2026-05 기준값 유지, 갱신 불가
