...
- [ ] **[윈도우 환경 필요]** 인포맥스 API 및 증권사 HTS 데이터 형식 확인
  - 실제 데이터 샘플 수집
  - 스키마 조정 필요 여부 확인
...
## 📦 Phase 2: 데이터 수집기 개발 (대기 중 - 윈도우 환경 필요)

### 🪟 윈도우 환경에서 먼저 할 작업 (최우선!)

- [ ] **데이터 형식 확인** ⚠️ **맥에서 불가, 회사 윈도우 컴퓨터 필요**
  - [ ] 인포맥스 API 문서 확인
  - [ ] 인포맥스 API 샘플 데이터 수집:
    - [ ] 종목 마스터 (5-10건)
    - [ ] 일봉 OHLCV (1종목, 최근 30일)
    - [ ] 투자자별 수급 (1종목, 최근 30일)
  - [ ] 증권사 HTS API 샘플 데이터 수집 (가능하면)
  - [ ] 샘플 데이터를 JSON/CSV 파일로 저장
  - [ ] 맥으로 전송 (이메일, 클라우드 등)

- [ ] **스키마 조정 여부 결정**
  - [ ] 샘플 데이터와 현재 스키마 비교
  - [ ] 필요시 `database/schema/init_schema.sql` 수정
  - [ ] ALTER TABLE 또는 재생성
