...
# 1. 기존 DB 삭제 후 재생성
echo "[1/2] DB 재생성..."
$PGBIN/psql -U $DB_USER -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '$DB_NAME' AND pid <> pg_backend_pid();" > /dev/null 2>&1 || true
$PGBIN/dropdb -U $DB_USER --if-exists $DB_NAME
$PGBIN/createdb -U $DB_USER $DB_NAME
echo "  ✅ DB 재생성 완료"

# 2. pg_restore
echo "[2/2] pg_restore 실행 중... (수 분 소요)"
