...
| 03:30 | update_listed_shares — 상장주식수(LS t1102) + **유동비율(wisereport)** | 일요일 |
| 03:30 | quarterly_sector — 업종 크롤 (**Naver**, 이전 FnGuide) | 분기 첫 일요일 (1/4/7/10월) |
| 04:00 | quarterly_financials | 분기 마감 후 첫 일요일 (4/6/9/12월) |
| **11:30** | **일일 요약 알림** (최근 24h, 이상 없어도 매일) | 매일 |
...
**알림은 3단계** (2026-08-03~): 실패=즉시+소리 / 이상 있는 성공(`⚠️`·`⛔` 자동 감지)=즉시 무음 / **깨끗한 성공=전송 안 함**. 대신 매일 11:30 일일 요약 1통. 요약은 이상이 없어도 보낸다 — **안 오는 것 자체가 이상 신호**.

상세 (요일별 동작, 2-pass 의미, 대기 로직, 동시성 가드, 일별 콜 분배, 알림 정책): **`docs/스케줄러_운영.md`**
...
프로세스 판별은 `/proc/<pid>/cmdline` 파싱(argv[0]=python + 인자에 스크립트 경로)이다. `pgrep -f`만 쓰면 **명령줄에 그 문자열이 든 셸까지 잡혀** 엉뚱한 PID의 시작 시각을 보고한다 — 재시작과 검증을 한 줄에서 돌릴 때 실제로 발생(2026-07-30). 프로세스가 2개 이상이면 중복 가동 경고 후 가장 오래된 것 기준으로 검사.
