...
- 옵션 1 권고: 23:00 — 22:30 stockfut와 30분 직렬(LENS 등 외부 LS 사용처가 22:30~24:00 한 블록만 피하면 됨) + 데이터 무결성 (정규장 마감 + 시간외 18:00 종료 후 5시간) + 사용자 활동 시간대
...
→ 주식선물은 매일 받지 않으면 영구 손실. 별도 22:30 KST cron 필요.
...
- `job_stockfut_today` (**22:30 KST 평일**) 신규 — 주식선물 t8406 당일 적재 (장 마감 후 사후호가/정산 끝난 시점)
...
22:30 KST  stockfut_today     주식선물 t8406 (당일만, historical 불가)
05:30 KST  daily_update       OHLCV/수급/외인 + 배당 + LENS export (인포맥스 + DART)
일03:00    weekly_backup      DB 백업
```

### 알려진 한계
- **주식선물 historical 불가** — 매일 22:30 cron 미실행 시 그날 영구 손실
