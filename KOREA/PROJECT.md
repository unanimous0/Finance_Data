...
> **마지막 업데이트**: 2026-02-19 (데이터 검증 및 스키마 정리 완료)
> **프로젝트 상태**: Phase 1 데이터 적재 및 검증 완료 ✅ → Phase 2 준비 중
...
- Phase 1 완료 ✅ (DB 구축, 핵심 모듈, 데이터 적재 및 검증)
  ...
  - 3,726개 종목 × OHLCV/시가총액/투자자/유동주식수 데이터
...
| floating_shares | 1,052,045건 | 2022-01-03 ~ 2026-02-19 | 2,546개 |
| **합계** | **20,599,751건** | **4년치** | - |
...
- `market_cap_daily.shares_outstanding` 삭제 (전부 NULL, 불필요)
- `investor_trading.net_buy_volume` 삭제 (전부 NULL)
- `investor_trading.buy_volume` 삭제 (전부 NULL)
- `investor_trading.sell_volume` 삭제 (전부 NULL)
- `investor_trading.buy_value` 삭제 (전부 NULL)
- `investor_trading.sell_value` 삭제 (전부 NULL)
- investor_trading에는 `net_buy_value`(순매수금액)만 유지
...
Hypertable: time 기준 자동 파티셔닝
컬럼: time, stock_code, investor_type, net_buy_volume, net_buy_value, buy_volume, sell_volume
```
