...
> **마지막 업데이트**: 2026-05-14
> **프로젝트 상태**: Phase 4 완료 + Phase 5(배당) + KRX 휴장일 SSoT + KOSPI200/KOSDAQ150 SCD2 + ETF 일별 스냅샷 + 지수/지수선물/주식선물 **일별** OHLCV + **Phase 6 분봉**: 종목/ETF 30초봉 ✅ (1분봉 백필 중) + **지수/지수선물/주식선물 30초봉 통합 (2026-05-13)** + **분봉 일배치 안정화 (2026-05-14)**: cron 23:00 이전 + LS API 5xx 회피 + 갭 fill 종목별화
...
**배경**: 인포맥스 API는 REST 기반으로 OS 무관 (macOS/Linux/Windows 모두 동작)

**결정**: 일반적인 주식 데이터 구조로 스키마 먼저 생성 → 실제 API 확인 후 조정 완료

### 인포맥스 API 데이터 제공 시간 (2026-03-17 실험)

**실험**: 18:10 KST 1차 수집 / 20:23 KST 2차 수집 비교

| 수집 시각 | OHLCV 성공률 | 비고 |
|---------|-------------|------|
| 18:10 KST (1차 직후) | 29% (1,111/3,803) | 70% 이상 데이터 미제공 |
| 20:23 KST (2차 후) | ~100% (2,346/2,347) | 472350 1건만 실패 |

**결론**: 인포맥스 1차 제공(16:30~16:40)은 일부 종목만 반영, **2차 제공(18:40+) 이후인 19:00 이후 수집 권장**

### API 미등록 종목 자동 비활성화 (2026-03-17)

**문제**: ETF 청산 등으로 사라진 종목이 `/api/stock/expired`에 미등록 → 자동 처리 불가

**해결**: `sync_stock_master()`에 3단계 추가
- 상장 API에도 없고, 상폐 API에도 없는 DB 활성 종목 → `is_active=FALSE`
- 매일 STEP 0에서 자동 처리 (ETF 청산 포함)
